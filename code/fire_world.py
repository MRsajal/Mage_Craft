...
def load_fire_map(width, height):
    ...
    Looks for a file named `fire_world.png` in the project's `images/` folder first,
    then the same folder as this module. If the file is missing or fails to load,
    returns a fallback placeholder surface.
    """
    candidates = []
    # project-level images folder (one level up from code/)
    proj_images = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "images", "fire_world.png"))
    candidates.append(proj_images)
    # module-local copy
    module_path = os.path.join(os.path.dirname(__file__), "fire_world.png")
